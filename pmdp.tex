...
%%%% THIS IS FALSE!!!!
% \begin{frame}{TCP/IP: Non-Explosion}

%   After the $k$-th jump the rate is $x_k = x_0 / 2^k$. Let $S_k$ denote
%   the $k$-th inter-jump time starting from $x_k$, and note that $S_k$ are
%   independent by the Markov property.

%   \vfill
%   Since $\phi(t,x_k) = x_k + ct$, the integrated hazard is exact:
%   \[
%     \Lambda(t,x_k) = \int_0^t \alpha(x_k + cs)\,\mathrm{d}s
%     = \alpha x_k t + \frac{\alpha c}{2}t^2,
%   \]
%   so:
%   \[
%     E[S_k] = \int_0^\infty \exp\!\left(-\alpha x_k t - \frac{\alpha c}{2}t^2\right)\mathrm{d}t.
%   \]
...
%   Since $x_k = x_0/2^k \to 0$, by monotone convergence:
%   \[
%     E[S_k] \;\xrightarrow{k\to\infty}\; \int_0^\infty \exp\!\left(-\frac{\alpha c}{2}t^2\right)\mathrm{d}t
%     = \sqrt{\frac{\pi}{2\alpha c}} \;>\; 0.
%   \]

%   \vfill

%   \begin{block}{Non-explosion}
%     Since $S_k \geq 0$ are independent and $E[S_k] \to \sqrt{\pi/(2\alpha c)} > 0$,
%     we have $\sum_k E[S_k] = \infty$, hence $\sum_k S_k = \infty$ a.s.
%     Therefore $T_n \to \infty$ a.s.\ and the process does not explode.
%   \end{block}
...
\begin{frame}{Ergodicity}
  ...
  \begin{definition}
    ...
    \[
      \int_E P_t f\,\mathrm{d}\pi = \int_E f\,\mathrm{d}\pi
...
\begin{frame}{Harris Recurrence: Connection to Ergodicity}
  ...
  \begin{block}{Harris ergodic theorem, Meyn \& Tweedie (1993)
                Theorem~13.0.1}
...
\begin{frame}{Foster's Criterion: Bound on Return Times}
  ...
  \begin{block}{Derivation via the Dynkin formula}
    ...
    \[
      ...
      \leq -\varepsilon\, E_x[\tau_C] + b\, E_x[\tau_C \wedge \sigma_C],
    \]
    where $\sigma_C$ is the first \emph{entry} time into $C$.
...
\begin{frame}{TCP/IP: Ergodicity via Foster's Criterion}
  ...
  \bigskip
  \textbf{Applying Foster's criterion.} Choose
  $C = \left[0,\, \sqrt{\frac{2c}{\alpha}} + \delta\right]$
  for any $\delta > 0$. Then:

  \begin{itemize}
    \setlength\itemsep{6pt}
    \item \textbf{Outside $C$:} for $x > \sqrt{2c/\alpha} + \delta$,
          \[
...
\begin{frame}[allowframebreaks]{References}
  ...
  \begin{itemize}
    ...
    \item F.\ Dufour, O.\,L.\,V.\ Costa.
          ...
          SIAM J.\ Control Optim.\ \textbf{37}:1483--1502, 2002.
